...
?>
 ...
 <h2 class='text-danger text-center' id='HEntree'>Les Desserts</h2>

<?php
//Ceci ce sont les cartes les menus de la page principal
echo "<div id='gestionCarte'>";

foreach($resultatRecupererIdRecetteDeTousLesDessert->fetchAll(PDO::FETCH_NUM) as $valeur)
{
    //echo $valeur[0];
    //echo '<br>';
  
    //echo $valeur[1];
    $resultatRecupererRecetteDeTousLesDessert=modelRecupererLesTroisPremiersDessert($valeur[0]);
    $resultatY=$resultatRecupererRecetteDeTousLesDessert->fetchAll(PDO::FETCH_ASSOC);
    foreach($resultatY as $valeur1)
    {
         //echo $valeur1['Nom'];
